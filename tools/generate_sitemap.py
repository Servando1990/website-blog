...
def traverse_content(
    root_dir: str = "src/content",
) -> Generator[Tuple[str, str, str], None, None]:
    """
    Recursively traverse the content folder and yield the path, content, and content hash of each file.

    Args:
        root_dir (str): The root directory to start traversing from. Defaults to 'src/content'.

    Yields:
        Tuple[str, str, str]: A tuple containing the relative path from root_dir, the file content, and the content hash.
    """
...
async def generate_sitemap(
    ...
    files_to_process = list(traverse_content(root_dir))
...
@app.command()
def main(
    root_dir: str = typer.Option("src/content", help="Root directory to traverse"),
